...
class DevAccessController extends Controller
{
    ...
    {
        if ($request->session()->get('developer_authenticated') || $request->session()->get('token') === '12345678') {
            return redirect('/');
        }
    ...
    public function sendOtp(Request $request)
    {
        ...
        // Fallback to DEV_ACCESS_EMAIL if DEV_ACCESS_EMAILS is empty
        if (empty($emailsString)) {
            $emailsString = env('DEV_ACCESS_EMAIL', '');
        }
        ...
        if (in_array($email, $allowedEmails)) {
            ...
            $request->session()->put([
                'dev_otp_email' => $email,
                'dev_otp_code' => 'DECOY_' . Str::random(10),
                'dev_otp_expires_at' => $expiresAt,
    ...
    public function verifyOtp(Request $request)
    {
        ...
        $request->session()->put('token', '12345678'); // set token for middleware compatibility
        $request->session()->put('dev_access_granted', true); // set for legacy middleware compatibility
    ...
    {
        return $this->clearPassword($request);
    }
