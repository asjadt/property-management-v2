    ...
    public function getTenants($perPage, Request $request)
    {
        try {
            ...
            $tenantQuery =  Tenant::leftJoin('property_tenants', 'tenants.id', '=', 'property_tenants.tenant_id')
                ...
                ->leftJoin('agreement_tenants', 'tenants.id', '=', 'agreement_tenants.tenant_id')
                ->leftJoin('tenancy_agreements', 'agreement_tenants.tenancy_agreement_id', '=', 'tenancy_agreements.id')
                ->where([
            ...
            if (!empty($request->property_id)) {
                $tenantQuery = $tenantQuery->where(function ($query) use ($request) {
                    $query->where('properties.id', $request->property_id)->orWhere('tenancy_agreements.property_id', $request->property_id);
                });
            }
